...
@config_enumerate
def model(hyperparameters, total_count_t, alt_count_t=None, z_t=None):
    alpha_pi_k, alpha_mu_k, beta_mu_k, lambda_nu_k = hyperparameters
    ...
    with pyro.plate('loci', total_count_t.shape[0]):
        ...
        alt_count_t = pyro.sample('alt_count_t', dist.BetaBinomial(alpha_k[z_t], beta_k[z_t], total_count_t), obs=alt_count_t)
    
    return pi_k, mu_k, nu_k, z_t, alt_count_t
...
def main():
    ...
    hyperparameters = alpha_pi_k, alpha_mu_k, beta_mu_k, lambda_nu_k
    ...
    def initialize(seed):
        ...
        global_guide = AutoDelta(
            poutine.block(model, expose=['pi_k', 'mu_k', 'nu_k'])
        )
    ...
    for param in ['pi_k', 'mu_k', 'nu_k']:
        param_map = map_estimates[param].data.numpy()
        logger.info(f'{param} = {param_map}')

    # calculate GQ and SQ
    data_ti = torch.tensor([[d, a, k] for d, a in zip(total_count_t, alt_count_t) for k in range(n_components)])
    map_model = poutine.condition(model, data=global_guide())
    map_model_tr = poutine.trace(map_model).get_trace(hyperparameters, data_ti[:, 0], data_ti[:, 1], data_ti[:, 2])
    map_model_tr.compute_log_prob()
    loglike_tk = (map_model_tr.nodes['z_t']['log_prob'] + map_model_tr.nodes['alt_count_t']['log_prob']).data.numpy().reshape(-1, n_components)
    gt_t = np.argmax(loglike_tk, axis=1)
    gq_t = 10. * np.diff(np.sort(np.log10(np.exp(loglike_tk)), axis=1), n=1, axis=1)[:, -1]
    sq_t = -10. * np.log10(np.exp(loglike_tk[:, 0] - torch.logsumexp(torch.tensor(loglike_tk), dim=1).data.numpy()))
