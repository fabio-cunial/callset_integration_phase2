...
for i=[1:N_CHILDREN]
    ...
    subplot(2,3,1); hold on;
    V1=load(sprintf('%s/gt_matrix_numneigh/%s_v1_all_gtmatrix.txt_numneigh_to_denovo_rate.txt',SOURCE_DIR,CHILDREN{i}));
    BB=load(sprintf('%s/gt_matrix_numneigh/%s_beta_binomial_all_gtmatrix.txt_numneigh_to_denovo_rate.txt',SOURCE_DIR,CHILDREN{i}));
    plot(X,V1,'.'); plot(X,BB,'o');
    axis square; grid on; axis([-1,11,0,0.6]); xlabel('NumNeighbors'); ylabel('De novo rate'); title('All calls'); set(gca,'fontsize',FONT_SIZE);
    
    subplot(2,3,4); hold on;
    V1=load(sprintf('%s/gt_matrix_numneigh/%s_v1_all_gtmatrix.txt_numneigh.txt',SOURCE_DIR,CHILDREN{i}));
    BB=load(sprintf('%s/gt_matrix_numneigh/%s_beta_binomial_all_gtmatrix.txt_numneigh.txt',SOURCE_DIR,CHILDREN{i}));
    plot(X,V1,'.'); plot(X,BB,'o');
    ...
    subplot(2,3,2); hold on;
    V1=load(sprintf('%s/gt_matrix_numneigh/%s_v1_tr_gtmatrix.txt_numneigh_to_denovo_rate.txt',SOURCE_DIR,CHILDREN{i}));
    BB=load(sprintf('%s/gt_matrix_numneigh/%s_beta_binomial_tr_gtmatrix.txt_numneigh_to_denovo_rate.txt',SOURCE_DIR,CHILDREN{i}));
    plot(X,V1,'.'); plot(X,BB,'o');
    axis square; grid on; axis([-1,11,0,0.6]); xlabel('NumNeighbors'); ylabel('De novo rate'); title('Inside TRs'); set(gca,'fontsize',FONT_SIZE);
    
    subplot(2,3,5); hold on;
    V1=load(sprintf('%s/gt_matrix_numneigh/%s_v1_tr_gtmatrix.txt_numneigh.txt',SOURCE_DIR,CHILDREN{i}));
    BB=load(sprintf('%s/gt_matrix_numneigh/%s_beta_binomial_tr_gtmatrix.txt_numneigh.txt',SOURCE_DIR,CHILDREN{i}));
    plot(X,V1,'.'); plot(X,BB,'o');
    ...
    subplot(2,3,3); hold on;
    V1=load(sprintf('%s/gt_matrix_numneigh/%s_v1_not_tr_gtmatrix.txt_numneigh_to_denovo_rate.txt',SOURCE_DIR,CHILDREN{i}));
    BB=load(sprintf('%s/gt_matrix_numneigh/%s_beta_binomial_not_tr_gtmatrix.txt_numneigh_to_denovo_rate.txt',SOURCE_DIR,CHILDREN{i}));
    plot(X,V1,'.'); plot(X,BB,'o');
    axis square; grid on; axis([-1,11,0,0.6]); xlabel('NumNeighbors'); ylabel('De novo rate'); title('Outside TRs'); set(gca,'fontsize',FONT_SIZE);
    legend('V1','BB','location','eastoutside');
    
    subplot(2,3,6); hold on;
    V1=load(sprintf('%s/gt_matrix_numneigh/%s_v1_not_tr_gtmatrix.txt_numneigh.txt',SOURCE_DIR,CHILDREN{i}));
    BB=load(sprintf('%s/gt_matrix_numneigh/%s_beta_binomial_not_tr_gtmatrix.txt_numneigh.txt',SOURCE_DIR,CHILDREN{i}));
    plot(X,V1,'.'); plot(X,BB,'o');
