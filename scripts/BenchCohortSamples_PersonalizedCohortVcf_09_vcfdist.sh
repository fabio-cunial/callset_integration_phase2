...
INPUT_DIR="/Users/fcunial/Downloads/BenchCohortSamples_PersonalizedCohortVcf/09_kanpig_1_0_2_vcfdist"
MIN_N_SAMPLES="2 4 8 16 32 64 128 256 512"
for REGION in all tr not_tr; do
    rm -f ${INPUT_DIR}/personalized_${REGION}.csv
    for SAMPLE in ${SAMPLES}; do
        P=$(grep ^SV ${INPUT_DIR}/${SAMPLE}_v1_07_${REGION}.txt | grep BEST | cut -f 8)
        R=$(grep ^SV ${INPUT_DIR}/${SAMPLE}_v1_07_${REGION}.txt | grep BEST | cut -f 9)
        F=$(grep ^SV ${INPUT_DIR}/${SAMPLE}_v1_07_${REGION}.txt | grep BEST | cut -f 10)
        ROW="${P},${R},${F}"
        for MIN in ${MIN_N_SAMPLES}; do
            P=$(grep ^SV ${INPUT_DIR}/${SAMPLE}_${MIN}_${REGION}.txt | grep BEST | cut -f 8)
            R=$(grep ^SV ${INPUT_DIR}/${SAMPLE}_${MIN}_${REGION}.txt | grep BEST | cut -f 9)
            F=$(grep ^SV ${INPUT_DIR}/${SAMPLE}_${MIN}_${REGION}.txt | grep BEST | cut -f 10)
            ROW="${ROW},${P},${R},${F}"
        done
