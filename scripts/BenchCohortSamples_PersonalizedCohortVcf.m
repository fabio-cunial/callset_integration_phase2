...
line ([1 1+length(MIN_N_SAMPLES)], [A(1,3) A(1,3)], "linestyle", "--", "color", "b");
line ([1 1+length(MIN_N_SAMPLES)], [A(1,4) A(1,4)], "linestyle", "--", "color", "r");
xticks([1:length(MIN_N_SAMPLES)+1]); xticklabels(LABELS); title('Inside TR, 07'); grid on; axis([0,length(LABELS)+1,0,1]); axis square; legend('precision 1.0.2','recall 1.0.2','precision 1.1.0','recall 1.1.0','location','southwest'); set(gca,'fontsize',FONT_SIZE);
