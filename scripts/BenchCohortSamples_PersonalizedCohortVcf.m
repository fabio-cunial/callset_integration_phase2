...
for i=[1:nrows]
    ...
    subplot(1,3,1); hold on; plot(1+X,P,'ob'); plot(1+X,R,'or'); plot(1+X,F,'og'); plot(1+X,C,'om');
endfor
...
xticks([1:length(MIN_N_SAMPLES)+1]); xticklabels(LABELS); xlabel('Min n. samples'); title('All calls, 07'); grid on; axis([0,length(LABELS)+1,0,1]); axis square; legend('precision 1.0.2','recall 1.0.2','f1 1.0.2','concordance 1.0.2','precision 1.1.0','recall 1.1.0','f1 1.1.0','concordance 1.1.0','location','southoutside'); set(gca,'fontsize',FONT_SIZE);
...
for i=[1:nrows]
    ...
    subplot(1,3,2); hold on; plot(1+X,P,'ob'); plot(1+X,R,'or'); plot(1+X,F,'og'); plot(1+X,C,'om');
endfor
line ([1 1+length(MIN_N_SAMPLES)], [A(1,1) A(1,1)], "linestyle", "--", "color", "b");
line ([1 1+length(MIN_N_SAMPLES)], [A(1,2) A(1,2)], "linestyle", "--", "color", "r");
line ([1 1+length(MIN_N_SAMPLES)], [A(1,3) A(1,3)], "linestyle", "--", "color", "g");
...
for i=[1:nrows]
    ...
    subplot(1,3,3); hold on; plot(1+X,P,'ob'); plot(1+X,R,'or'); plot(1+X,F,'og'); plot(1+X,C,'om');
endfor
line ([1 1+length(MIN_N_SAMPLES)], [A(1,1) A(1,1)], "linestyle", "--", "color", "b");
line ([1 1+length(MIN_N_SAMPLES)], [A(1,2) A(1,2)], "linestyle", "--", "color", "r");
line ([1 1+length(MIN_N_SAMPLES)], [A(1,3) A(1,3)], "linestyle", "--", "color", "g");
