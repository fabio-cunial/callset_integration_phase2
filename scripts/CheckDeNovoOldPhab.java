 ...
 * 
 */
public class CheckDeNovoOldPhab {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean isMissing, onAutosome, isAltChild, isDeNovo;
        int i, j;
        int nTrios, nRecords;
        String str;
        BufferedReader br;
        ...
        while (str!=null) { 
            ...
            for (i=0; i<nTrios; i++) {
                ...
                isMissing=false; onAutosome=true;
                for (j=3*i; j<3*i+3; j++) {
                    try { 
                        if (tokens[j].indexOf(".")>=0) isMissing=true;
                        if (tokens[j].length()!=3) onAutosome=false;
                    } catch(Exception e) { System.err.println("ERROR in trio "+i+"-th: "+str); }
                }
                if (isMissing || !onAutosome) continue;
                denominator[i]++;
