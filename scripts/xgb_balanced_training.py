...
def train(labeled_annotations_file,
          unlabeled_annotations_file,
          hyperparameters_json_file,
          output_prefix,
          force_balanced_training=False):
    logger.info('Reading annotations...')
    labeled_annotation_names_i, labeled_X_ni = read_annotations(labeled_annotations_file)
    unlabeled_annotation_names_i, unlabeled_X_ni = read_annotations(unlabeled_annotations_file)
    if force_balanced_training and len(unlabeled_X_ni) > len(labeled_X_ni):
        logger.warning(f'Forcing balanced training ({len(labeled_X_ni)} labeled, {len(unlabeled_X_ni)} unlabeled).')
        unlabeled_X_ni = unlabeled_X_ni[:len(labeled_X_ni), :]
    assert all(labeled_annotation_names_i == unlabeled_annotation_names_i), \
        'Annotation names differ in labeled and unlabeled annotations.'
    annotation_names_i = labeled_annotation_names_i
    X_ni = np.concatenate([labeled_X_ni, unlabeled_X_ni])
    y_n = np.concatenate([np.ones(len(labeled_X_ni)), np.zeros(len(unlabeled_X_ni))])
    ...
    logger.info(f'Training XGBoost with {imputed_X_ni.shape[0]} training sites x {imputed_X_ni.shape[1]} annotations...')
    clf = xgboost.XGBClassifier(**hyperparameters_kwargs)
    clf.fit(imputed_X_ni, y_n)
    ...
    def score_samples(test_annotation_names_i,
        ...
        return clf.predict_proba(imputer.transform(test_X_ni))[:, 1]
...
def main():
    ...
    parser.add_argument('--unlabeled_annotations_file',
                        type=str,
                        required=False,
                        help='')
    ...
    unlabeled_annotations_file = args.unlabeled_annotations_file

    # this script can handle both training and scoring; we check the passed arguments to determine which is appropriate
    if args.unlabeled_annotations_file is not None and args.hyperparameters_json_file is not None and args.output_prefix is not None and \
        ...
        train(annotations_file,
              ...
              hyperparameters_json_file,
              output_prefix,
              args.force_balanced_training)
    elif args.unlabeled_annotations_file is None and args.hyperparameters_json_file is None and args.output_prefix is None and \
