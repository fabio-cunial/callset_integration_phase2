...
set -euxo pipefail
export LC_ALL=C  # To make sort not ignore commas/tabs based on locale
N_THREADS="8"
...
function DownloadVcf() {
    ...
    bcftools query --format '%CHROM,%POS,%ID,%REF,%ALT\n' ${SAMPLE_ID}_${SUFFIX}_lt_50.bcf | sort > ${SAMPLE_ID}_${SUFFIX}_lt_50.csv
    bcftools query --format '%CHROM,%POS,%ID,%REF,%ALT\n' ${SAMPLE_ID}_${SUFFIX}_ge_50.bcf | sort > ${SAMPLE_ID}_${SUFFIX}_ge_50.csv
}
...
function CompareOldNew() {
    ...
    comm -2 -3 ${OLD_CSV} ${NEW_CSV} > ${SAMPLE_ID}_${SVLEN_STRING}_old_only.csv
    comm -1 -3 ${OLD_CSV} ${NEW_CSV} > ${SAMPLE_ID}_${SVLEN_STRING}_new_only.csv
    comm -1 -2 ${OLD_CSV} ${NEW_CSV} > ${SAMPLE_ID}_${SVLEN_STRING}_common.csv
    OLD_ONLY=$(wc -l < ${SAMPLE_ID}_${SVLEN_STRING}_old_only.csv)
    NEW_ONLY=$(wc -l < ${SAMPLE_ID}_${SVLEN_STRING}_new_only.csv)
    COMMON=$(wc -l < ${SAMPLE_ID}_${SVLEN_STRING}_common.csv)

    # Studying the records that are missing from OLD
    cut -d , -f 3 ${SAMPLE_ID}_${SVLEN_STRING}_new_only.csv > ${SAMPLE_ID}_${SVLEN_STRING}_new_only_ids.txt
...
for SAMPLE_ID in ${SAMPLE_IDS}; do
    ...
    CompareOldNew ${SAMPLE_ID} lt_50 ${SAMPLE_ID}_old_lt_50.csv ${SAMPLE_ID}_new_lt_50.csv results_lt_50.csv
    CompareOldNew ${SAMPLE_ID} ge_50 ${SAMPLE_ID}_old_ge_50.csv ${SAMPLE_ID}_new_ge_50.csv results_ge_50.csv
done
