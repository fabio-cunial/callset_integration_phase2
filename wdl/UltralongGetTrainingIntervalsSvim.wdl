...
# a set of corresponding filtered and canonized svim-asm VCFs, the program 
# selects, for each sample, the query records that match svim-asm.
#
# Remark: sequence similarity is not used to decide a match.
#
workflow UltralongGetTrainingIntervalsSvim {
    input {
        File samples_tsv
        String suffix = "del"
        File reference_fai
    ...
    parameter_meta {
        ...
        remote_indir_truth: "Without final slash. Contains per-sample canonized and filtered svim-asm VCFs."
    ...
    command <<<
        ...
        function GetTrainingIntervalsThread() {
            ...
            while read -u 3 LINE; do
                ...
                bcftools filter --include 'SVTYPE=="'${SVIM_SVTYPE}'"' --output-type z ${SAMPLE_ID}_canonized.vcf.gz --output ${SAMPLE_ID}_truth.vcf.gz
                rm -f ${SAMPLE_ID}_canonized.vcf.gz*
                bcftools index --threads ${N_THREADS} -f -t ${SAMPLE_ID}_truth.vcf.gz
                
                # Computing matches. Sequence similarity is never used.
                ${TIME_COMMAND} truvari bench -b ${SAMPLE_ID}_truth.vcf.gz -c ${SAMPLE_ID}_query.vcf.gz --sizemin 1 --sizemax ${INFINITY} --sizefilt 1 --refdist ~{truvari_refdist} ${CHUNKSIZE_FLAG} --pctseq 0 --pctsize ~{truvari_pctsize} --pctovl ~{truvari_pctovl} --pick single ${TRUVARI_EXTRA_FLAGS} -o ./${SAMPLE_ID}_truvari/
                ...
                rm -rf ${SAMPLE_ID}_query.vcf.gz* ${SAMPLE_ID}_truth.vcf.gz* ${SAMPLE_ID}_truvari*/
