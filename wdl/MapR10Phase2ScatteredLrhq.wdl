...
task Minimap2 {
    ...
    command <<<
        ...
        ls -laht
        df -h
        ${TIME_COMMAND} ~{docker_dir}/minimap2/minimap2 -t ${N_THREADS} -K4G -x lr:hq -ayYL --MD --eqx --cs ~{reference_fa} -R ${FAKE_RG} ~{reads_fastq_gz} > out.sam
        ls -laht
        df -h
        ${TIME_COMMAND} samtools view -@ ${N_THREADS} -b out.sam > out.bam
        ls -laht
        df -h
    >>>
    ...
    runtime {
        docker: "fcunial/callset_integration_phase2_minimap2r10"
        cpu: n_cpus
        memory: ram_size_gb + "GB"
        disks: "local-disk " + disk_size_gb + " HDD"
        preemptible: 2
