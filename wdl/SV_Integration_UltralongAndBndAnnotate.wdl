...
# A simple fusion of `SV_Integration_UltralongAnnotate.wdl` and 
# `SV_Integration_BndAnnotate.wdl`, to avoid paying the cost of localizing the 
# same BAM twice on thousands of production samples. See the original workflows
# for details.
#
# Minor imperfection: while training the models, we erroneously used a different
# version of `feature_extraction.py` for BNDs and ultralong, so we must use
# exactly the same setup in production. Ultralong used the file in the docker, 
# with all features normalized; BNDs used the version in the workspace bucket, 
# with some features not normalized. Diffs affect features `depth_mad`, 
# `split_reads`, `homopolymer_max`: the first two are always zero for BNDs in 
# practice, and the last one is an absolute count over a window of constant 
# length, so it is already implicitly normalized.
#
workflow SV_Integration_UltralongAndBndAnnotate {
    input {
        File chunk_tsv
        String remote_indir
        ...
        String custom_min_indel_length = "500"
...
task Impl {
    ...
    command <<<
        ...
        # Remark: this function localizes the BAM.
        #
        # @param 
        # $2 A row of `chunk_tsv`.
        #
        function LocalizeSample_Ultralong() {
            local SAMPLE_ID=$1
            local LINE=$2
        ...
        # Remark: this function does not localize the BAM.
        #
        # @param 
        # $2 A row of `chunk_tsv`.
        #
        function LocalizeSample_Bnd() {
            local SAMPLE_ID=$1
            local LINE=$2
        ...
        # Deletes all files related to the given sample, except the BAM.
        #
        function DelocalizeSample_Ultralong() {
            local SAMPLE_ID=$1
            
            rm -f ${SAMPLE_ID}*.bcf* ${SAMPLE_ID}*.vcf* ${SAMPLE_ID}*.tsv* ${SAMPLE_ID}*.csv* ${SAMPLE_ID}*.txt
        }


        # Deletes all files related to the given sample, including the BAM.
        #
        function DelocalizeSample_Bnd() {
            local SAMPLE_ID=$1
        ...
        #
        function CanonizeVcf_Ultralong() {
            local SAMPLE_ID=$1
...
END
        ...
        # set: UNMAP, SECONDARY, QCFAIL, DUP
        #
        function AnnotateCoverageBins_Bnd() {
            local SAMPLE_ID=$1
            local INPUT_VCF=$2
            local INPUT_BAM=$3
            local BREAKPOINT_WINDOW_BP=$4
        ...
        cat << 'END' > annotate_mapq_secondary_ultralong.sh
#!/bin/bash
set -euxo pipefail
...
END
        chmod +x annotate_mapq_secondary_ultralong.sh
        ...
        function AnnotateMapqSecondary_Interval() {
            ...
            ${TIME_COMMAND} xargs --arg-file=${SAMPLE_ID}_bins.wsv --max-lines=1 --max-procs=${N_THREADS} ./annotate_mapq_secondary_ultralong.sh ${INPUT_BAM}
        ...
        cat << 'END' > annotate_mapq_secondary_bnd.sh
#!/bin/bash
set -euxo pipefail
...
END
        chmod +x annotate_mapq_secondary_bnd.sh
        ...
        # `AnnotateClippedAlignments()`.
        #
        function AnnotateMapqSecondary_Bnd() {
            local SAMPLE_ID=$1
            local INPUT_VCF=$2
            local INPUT_BAM=$3
            local BREAKPOINT_WINDOW_BP=$4
            ...
            ${TIME_COMMAND} xargs --arg-file=${SAMPLE_ID}_bins.wsv --max-lines=1 --max-procs=${N_THREADS} ./annotate_mapq_secondary_bnd.sh ${INPUT_BAM}
            rm -f ${SAMPLE_ID}_bins.wsv
            ${TIME_COMMAND} bcftools query --format '%ID\n' ${INPUT_VCF} | sort | uniq > ${SAMPLE_ID}_variantID_sorted.txt
            rm -f ${SAMPLE_ID}_counts.tsv
            while read -u 6 ID; do
        ...
        cat << 'END' > annotate_clipped_alignments_1_ultralong.sh
#!/bin/bash
set -euxo pipefail
...
END
        chmod +x annotate_clipped_alignments_1_ultralong.sh


        cat << 'END' > annotate_clipped_alignments_1_bnd.sh
#!/bin/bash
set -euxo pipefail
...
END
        chmod +x annotate_clipped_alignments_1_bnd.sh
...
END
        ...
        cat << 'END' > annotate_clipped_alignments_2_bnd.sh
#!/bin/bash
set -euxo pipefail
...
END
        chmod +x annotate_clipped_alignments_2_bnd.sh
        ...
        function AnnotateClippedAlignments_Interval() {
            ...
            ${TIME_COMMAND} java -cp ~{docker_dir} UltralongIntervalGetBins ${INPUT_VCF} ~{reference_fai} 0 ${BREAKPOINT_WINDOW_BP} | tr '\t' ' ' > ${SAMPLE_ID}_bins.wsv
            ${TIME_COMMAND} xargs --arg-file=${SAMPLE_ID}_bins.wsv --max-lines=1 --max-procs=${N_THREADS} ./annotate_clipped_alignments_1_ultralong.sh ${INPUT_BAM} ~{docker_dir} ${MIN_CLIP_LENGTH} ${MIN_INDEL_LENGTH} ${MIN_MAPQ}
            rm -f ${SAMPLE_ID}_bins.wsv
        ...
        # annotates it with clipped alignment measures extracted from a BAM.
        #
        function AnnotateClippedAlignments_Bnd() {
            local SAMPLE_ID=$1
            local INPUT_VCF=$2
            local INPUT_BAM=$3
            local BREAKPOINT_WINDOW_BP=$4
            ...
            ${TIME_COMMAND} xargs --arg-file=${SAMPLE_ID}_bins.wsv --max-lines=1 --max-procs=${N_THREADS} ./annotate_clipped_alignments_1_bnd.sh ${INPUT_BAM} ~{docker_dir} ${MIN_CLIP_LENGTH} ${MIN_INDEL_LENGTH} ${MIN_MAPQ}
            rm -f ${SAMPLE_ID}_bins.wsv
            ${TIME_COMMAND} bcftools query --format '%ID\n' ${INPUT_VCF} > ${SAMPLE_ID}_variantID.txt
            rm -f *_counts.txt
            ${TIME_COMMAND} xargs --arg-file=${SAMPLE_ID}_variantID.txt --max-lines=1 --max-procs=${N_THREADS} ./annotate_clipped_alignments_2_bnd.sh ~{docker_dir} ${ADJACENCY_SLACK_BP} ${MEAN_COVERAGE} ${NORMALIZATION_MODE} ${MIN_INDEL_LENGTH}
            rm -f ${SAMPLE_ID}_variantID.txt
            cat *_counts.txt | sort -k 1,1 > ${SAMPLE_ID}_counts.tsv
            rm -f *_counts.txt
            ${TIME_COMMAND} bcftools query --format '%CHROM\t%POS\t%REF\t%ALT\t%ID\n' ${INPUT_VCF} | sort -k 5,5 > ${SAMPLE_ID}_chrom_pos_ref_alt_id.tsv
        ...
        function AnnotateCustom_Bnd() {
            ...
            for MAPQ in ${MAPQS}; do
                AnnotateCoverageBins_Bnd ${SAMPLE_ID} ${SAMPLE_ID}_in.vcf ${SAMPLE_ID}.bam ~{custom_breakpoint_window_bp} ${MEAN_COVERAGE} ${MAPQ}
                rm -f ${SAMPLE_ID}_in.vcf ; mv ${SAMPLE_ID}_annotated.vcf ${SAMPLE_ID}_in.vcf
            done
            AnnotateMapqSecondary_Bnd ${SAMPLE_ID} ${SAMPLE_ID}_in.vcf ${SAMPLE_ID}.bam ~{custom_breakpoint_window_bp}
            rm -f ${SAMPLE_ID}_in.vcf ; mv ${SAMPLE_ID}_annotated.vcf ${SAMPLE_ID}_in.vcf
            for MAPQ in ${MAPQS}; do
                AnnotateClippedAlignments_Bnd ${SAMPLE_ID} ${SAMPLE_ID}_in.vcf ${SAMPLE_ID}.bam ~{custom_breakpoint_window_bp} ~{custom_adjacency_slack_bp} ~{custom_min_clip_length} ~{custom_min_indel_length} ~{custom_breakpoint_window_slack_bp} ${MEAN_COVERAGE} ${MAPQ} ${NORMALIZATION_MODE}
                rm -f ${SAMPLE_ID}_in.vcf ; mv ${SAMPLE_ID}_annotated.vcf ${SAMPLE_ID}_in.vcf
            done
        ...
        #
        cat << 'END' > feature_extraction_thread_ultralong.sh
#!/bin/bash
set -euxo pipefail
...
END
        chmod +x feature_extraction_thread_ultralong.sh


        cat << 'END' > feature_extraction_thread_bnd.sh
#!/bin/bash
set -euxo pipefail
...
END
        chmod +x feature_extraction_thread_bnd.sh
        ...
        #
        function FeatureExtraction_Ultralong() {
            local SAMPLE_ID=$1
            local INPUT_VCF=$2
            ...
            if [ ${PARALLELIZE} -eq 1 ]; then
                ...
                ${TIME_COMMAND} xargs --arg-file=list.txt --max-lines=1 --max-procs=${N_THREADS} ./feature_extraction_thread_ultralong.sh ${ALIGNMENTS_BAM}
                ls -laht 1>&2
        ...
        # samtools. For simplicity we leave this to the future.
        #
        function FeatureExtraction_Bnd() {
            local SAMPLE_ID=$1
            local INPUT_VCF=$2
            local ALIGNMENTS_BAM=$3
            local PARALLELIZE=$4
            ...
            if [ ${PARALLELIZE} -eq 1 ]; then
                ...
                rm -f ${SAMPLE_ID}_header.txt
                ls ${SAMPLE_ID}_chunk_*_vcf > list.txt
                ${TIME_COMMAND} xargs --arg-file=list.txt --max-lines=1 --max-procs=${N_THREADS} ./feature_extraction_thread_bnd.sh ${ALIGNMENTS_BAM}
                ls -laht 1>&2
                rm -f ${SAMPLE_ID}_feature_extraction_features.csv
                for FILE in $( ls ${SAMPLE_ID}_chunk_*_features.csv | sort -V ); do
                    cat ${FILE} >> ${SAMPLE_ID}_feature_extraction_features.csv
        ...
        function VcfToBed_StartEnd_Bnd() {
            local SAMPLE_ID=$1
            local INPUT_VCF=$2
...
END
        ...
        while read -u 3 LINE; do
            ...
            # -------------------------- Ultralong -----------------------------
            LocalizeSample_Ultralong ${SAMPLE_ID} ${LINE}
            df -h 1>&2
            ...
            if [ ${N_RECORDS} -eq 0 ]; then
                ...
                CanonizeVcf_Ultralong ${SAMPLE_ID} ${SAMPLE_ID}.vcf.gz
                ...
                FeatureExtraction_Ultralong ${SAMPLE_ID} ${SAMPLE_ID}_in.vcf ${SAMPLE_ID}.bam 0
            ...
            DelocalizeSample_Ultralong ${SAMPLE_ID}
            ...
            LocalizeSample_Bnd ${SAMPLE_ID} ${LINE}
            df -h 1>&2
            N_RECORDS=$( bcftools index --nrecords ${SAMPLE_ID}.vcf.gz.tbi )
            if [ ${N_RECORDS} -eq 0 ]; then
                ...
                CanonizeVcf_Bnd ${SAMPLE_ID} ${SAMPLE_ID}.vcf.gz
                
                # 2. Custom annotations
                AnnotateCustom_Bnd ${SAMPLE_ID} ${SAMPLE_ID}_canonized.vcf ${MEAN_COVERAGE} ~{min_mapq} 0

                # 3. Repeat tracks
                VcfToBed_StartEnd_Bnd ${SAMPLE_ID} ${SAMPLE_ID}_canonized.vcf
                ...
                # are both slow and use threads inefficiently. In practice this does
                # not decrease total runtime.
                FeatureExtraction_Bnd ${SAMPLE_ID} ${SAMPLE_ID}_canonized.vcf ${SAMPLE_ID}.bam 0
            ...
            DelocalizeSample_Bnd ${SAMPLE_ID}
            ls -laht 1>&2
