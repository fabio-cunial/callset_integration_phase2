...
# Kanpig vs Beta binomial
#
workflow BenchCohortSamplesBetaBinomial {
    input {
        ...
        Array[File] single_sample_betabinomial_vcf_gz
    ...
    scatter (i in range(length(sample_ids))) {
        call BenchSample {
            input:
                ...
                single_sample_betabinomial_vcf_gz = single_sample_betabinomial_vcf_gz[i],
...
task BenchSample {
    input {
        ...
        File single_sample_betabinomial_vcf_gz
    ...
    Int disk_size_gb = 10*( ceil(size(single_sample_kanpig_vcf_gz,"GB")) + ceil(size(single_sample_betabinomial_vcf_gz,"GB")) + ceil(size(single_sample_dipcall_vcf_gz,"GB")) )
    ...
    command <<<
        ...
        # Preprocessing the VCF after:
        # 1. intra-sample truvari -> kanpig
        # 2. kanpig -> beta binomial
        ${TIME_COMMAND} bcftools filter --threads ${N_THREADS} --include 'COUNT(GT="alt")>0' --output-type z ~{single_sample_betabinomial_vcf_gz} > sample_beta.vcf.gz
        ${TIME_COMMAND} tabix -f sample_beta.vcf.gz
        
        # Benchmarking
        if [ ~{bench_method} -eq 0 ]; then
            # Parallel for truvari (which is single-core).
            bench_thread sample_kanpig.vcf.gz kanpig &
            bench_thread sample_beta.vcf.gz beta &
            wait
        else
        ...
        # Outputting
        while : ; do
            TEST=$(gsutil -m cp '*.txt' ~{remote_output_dir} && echo 0 || echo 1)
            if [ ${TEST} -eq 1 ]; then
